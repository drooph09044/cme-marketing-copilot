...
export default function QARunsTab({
  ...
  async function runAll() {
    ...
    try {
      for (const p of profiles) {
        if (!results[p.id] && !running.has(p.id)) await simulate(p);
      }
    } finally {
    ...
    <div className="qa-wb-wrap">
      ...
      <div className="qa-wb">
        ...
        <section className="qa-wb__col">
          <div className="qa-wb__head">
            <h3>QA Suites</h3>
            <button type="button" className={`jo-btn jo-btn--primary qa-wb__runall${runAllLoading ? " is-loading" : ""}`} onClick={runAll} disabled={runAllLoading}>
              {runAllLoading ? (
                <><span className="qa-runall__spin" />Running…</>
              ) : "Run All"}
            </button>
          </div>
          {selected && (
