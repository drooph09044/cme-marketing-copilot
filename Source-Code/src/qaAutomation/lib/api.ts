...
const QA_API = "/api/qa-automation";
...
export const api = {
  ...
  subscribeRun(
    ...
    const es = new EventSource(`${QA_API}/runs/${runId}/stream`);
  ...
  async listSegments(journeyId?: string): Promise<Segment[]> {
    return http<Segment[]>(`/segments${journeyId ? `?journeyId=${encodeURIComponent(journeyId)}` : ""}`);
  },
