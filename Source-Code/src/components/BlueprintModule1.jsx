...
function ApiCampaignsJourneysPanel({ report }) {
  if (!report?.summary) {
    return null;
  }
...
function CampaignJourneyReport({ journeys = [], apiReport = null }) {
  if (apiReport?.summary) {
    return <ApiCampaignsJourneysPanel report={apiReport} />;
  }
  ...
  const report = useMemo(() => {
    ...
    const channelMix = Object.entries(channelCounts)
      .map(([label, value], index) => ({
        ...
        value,
        color:
          ["#2f84dc", "#22a979", "#7c73dd", "#66a329", "#f59e0b", "#14b8a6"][
          index % 6
          ],
      }))
  ...
  const metricCards = [
    {
      ...
      sub: "from blueprint API",
      color: "#2f84dc",
      icon: "🧭",
    },
    {
      label: "Campaigns",
      value: report.totalCampaigns,
      sub: "unique campaign IDs",
      color: "#22a979",
      icon: "🎯",
    },
    {
    ...
    },
    {
      ...
      sub: `${report.inactiveJourneys} inactive`,
      color: "#10b981",
      icon: " ",
    },
    {
  ...
  <CPieChart
    ...
    height={310}
    title="Channel mix"
    note="Channel distribution calculated from journey touchpoints."
    centerLabel=""
    data={report.channelMix}
    showLegend={false}
    showCenter={false}
  />
</div>
          ...
          <div className="cj-card-body">
            <div className="cj-suppression-list">
              {report.suppressionRules.length > 0 ? (
                report.suppressionRules.map((rule, index) => (
                  <div
    ...
    <section className="module-layout blueprint-layout">
      <aside className="panel side-panel narrow editor-panel">
        ...
        <div className="panel-body">
          <button type="button" className="button secondary full journey-back-button" onClick={() => onJourneyCategoryChange("")}>
            Back to Campaigns & Journeys
          </button>
          ...
          <div className="group-box">
            ...
            <label className="field compact">
              <span className="field-label small">Orchestration mode</span>
              <select
                className="field-input"
                value={form.orchestrationType ?? "journey"}
                onChange={(event) => onFormChange("orchestrationType", event.target.value)}
              >
                <option value="journey">Multi-touch journey</option>
                <option value="single-touchpoint">Single touchpoint campaign</option>
              </select>
            </label>
